...
/**
 * В отличии от UserActionMem UserAction не принимает параметров и предназначен для формирования
 * меню только на основе названия пункта. Номера пунктов необходимо формировать с помощью
 * внешнего цикла.
 *
 * Перечень пунктов меню задается по возрастанию. Пункт меню Exit program рекомендуется
 * перенести в позицию 0 для удобства. Для примера:
 * UserAction[] actions = {
 *                     new ExitAction(),
 *                     new CreateAction(),
 *                     ...
